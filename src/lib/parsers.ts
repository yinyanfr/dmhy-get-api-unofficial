...
export function parseDMHYSearchOptions(): SearchOptions {
...
export function parseDMHYList(html: string): EelItem[] {
  ...
  const table: HTMLTableElement | null = document.querySelector('table#topic_list');
  ...
  if (table?.tBodies) {
    for (const item of table.tBodies) {
      for (const row of item.rows) {
        ...
        const title = row.querySelector(`td.title a:nth-child(${group ? 2 : 1})`);
        item.title = converter(trim(title?.innerHTML?.replace(/\n */g, '')) || '');
        item.id = title
          ?.getAttribute('href')
          ?.match(/[^/]+\.html$/)?.[0]
          ?.replace('.html', '');
